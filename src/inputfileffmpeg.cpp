	...
	m_outputSamplerate(options.outputSampleRate)
{
	av_channel_layout_from_mask(
		...
		options.outputChannelLayout == InputFileOptions::MONO ? AV_CH_LAYOUT_MONO : AV_CH_LAYOUT_STEREO
	);
...
bool InputFileFFmpeg::openInternal(const char* filename, double startPosSeconds, double playTimeSeconds)
{
	...
	logInfo(
		"Opened file: %s; Codec: %s, Channels: %i, Rate: %i, Format: %s, Timebase: %i/%i, Sample-Estimation: %lld",
...
int InputFileFFmpeg::handleDecoded(AVFrame* frame, SampleProducer* sb)
{
	...
	if (frame && m_nextSeekTimestamp > 0)
	{
		...
		if (tsToSkip > 0)
		{
			...
			logInfo(
				"Current timestep is %f but desired is %f, skipping %lli samples", curTs * timeBase,
				m_nextSeekTimestamp * timeBase, m_skipSamples
			);
		}
		m_nextSeekTimestamp = 0;
	}
	...
	{
		res = swr_convert(
			m_swrCtx, &m_outBuf, OUTPUT_BUFFER_COUNT, frame ? (const uint8_t**)frame->extended_data : nullptr,
		...
		!m_done && res == OUTPUT_BUFFER_COUNT
	); // If we filled the whole output buffer, there might be more data to convert, so try again immediately
...
int InputFileFFmpeg::readSamples(SampleProducer* sampleBuffer)
{
	...
	while (!m_done && written == 0)
	{
		...
		// Patch missing PTS (can happen with some formats/codecs, e.g. MP3) by using DTS, which is usually set if PTS
		// is not.
