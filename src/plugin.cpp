...
extern "C"
{
	...
	int ts3plugin_init()
	{
		...
		/* -2 is a very special case and should only be used if a plugin displays a dialog (e.g. overlay) asking the
		 * user to disable the plugin again, avoiding the show another dialog by the client telling the user the plugin
		 * failed to load. For normal case, if a plugin really failed to load because of an error, the correct return
		 * value is 1. */
	}
	...
	/*
	 * If the plugin wants to use error return codes, plugin commands, hotkeys or menu items, it needs to register a
	 * command ID. This function will be automatically called after the plugin was initialized. This function is
	 * optional. If you don't use these features, this function can be omitted. Note the passed pluginID parameter is no
	 * longer valid after calling this function, so you must copy it and store it in the plugin.
	 */
...
#define END_CREATE_MENUS (*menuItems)[n++] = nullptr; assert(n == sz);
	...
	/*
	 * Menu IDs for this plugin. Pass these IDs when creating a menuitem to the TS3 client. When the menu item is
	 * triggered, ts3plugin_onMenuItemEvent will be called passing the menu ID of the triggered menu item. These IDs are
	 * freely choosable by the plugin author. It's not really needed to use an enum, it just looks prettier.
	 */
	enum
	{
	 ...
	 * This function is called after ts3plugin_init and ts3plugin_registerPluginID. A pluginID is required for plugin
	 * menus to work. Both ts3plugin_registerPluginID and ts3plugin_freeMemory must be implemented to use menus. If
	 * plugin menus are not used by a plugin, do not implement this function or return NULL.
	 */
	void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon)
	{
		 ...
		 * Icons are loaded from a subdirectory in the TeamSpeak client plugins folder. The subdirectory must be named
		 * like the plugin filename, without dll/so/dylib suffix e.g. for "test_plugin.dll", icon "1.png" is loaded from
		...
		/*
		 * Specify an optional icon for the plugin. This icon is used for the plugins submenu within context and main
		 * menus If unused, set menuIcon to NULL
		 */
		*menuIcon = (char*)malloc(PLUGIN_MENU_BUFSZ * sizeof(char));
		 ...
		 * Menus are enabled by default. Please note that shown menus will not automatically enable or disable when
		 * calling this function to ensure Qt menus are not modified by any thread other the UI thread. The enabled or
		 * disable state will change the next time a menu is displayed.
		 */
		/* For example, this would disable MENU_ID_GLOBAL_2: */
		/* ts3Functions.setPluginMenuEnabled(pluginID, MENU_ID_GLOBAL_2, 0); */

		/* All memory allocated in this function will be automatically released by the TeamSpeak client later by calling
...
#define END_CREATE_HOTKEYS (*hotkeys)[n++] = nullptr; assert(n == sz);
	...
	{
		// logDebug("onUpdateClientEvent: serverId = %i, clientId = %i, invokerID = %i, invokerName = \"%s\", invokerUID
		// = \"%s\"", 	(int)serverConnectionHandlerID, (int)clientID, (int)invokerID, invokerName ? invokerName : "NULL",
		//invokerUniqueIdentifier ? invokerUniqueIdentifier : "NULL");
		anyID myId = 0;
		...
		int inputState = 0;
		if (checkError(
				ts3Functions.getClientSelfVariableAsInt(
					serverConnectionHandlerID, (size_t)CLIENT_INPUT_HARDWARE, &inputState
				),
				"getClientSelfVariableAsInt error"
			))
			return;
		...
		{
			int oldInputState =
				clientInputHardwareStateMap[serverConnectionHandlerID]; // Gets initialized to 0 when nonexistant
			if (!oldInputState)
			{
	...
	/*
	 * Called when a plugin menu item (see ts3plugin_initMenus) is triggered. Optional function, when not using plugin
	 ...
	 * - selectedItemID: Channel or Client ID in the case of PLUGIN_MENU_TYPE_CHANNEL and PLUGIN_MENU_TYPE_CLIENT. 0 for
	 * PLUGIN_MENU_TYPE_GLOBAL.
	 */
	void ts3plugin_onMenuItemEvent(
		uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID
	)
	{
		switch (type)
		{
	...
	void ts3plugin_onTalkStatusChangeEvent(
		uint64 serverConnectionHandlerID, int status, int isReceivedWhisper, anyID clientID
	)
	{
		// logDebug("onTalkStatusChangeEvent status=%i, isReceivedWhisper=%i, clientID=%i", status, isReceivedWhisper,
		// (int)clientID);
		anyID myId = 0;
		if (checkError(ts3Functions.getClientID(serverConnectionHandlerID, &myId), "getClientID error"))
			return;
